...
from prometheus.agents.dqn.base import DQNAgentBase
...
class DQNTrainerBase(BaseTrainer):
    """
    通用 DQN 训练器

    支持所有 DQN 变体（标准 DQN, Double DQN, Dueling DQN, Rainbow）
    """
    ...
    def train(
        ...
        if len(action_dim) == 0:
            action_dim = env.spec.action_space.shape
        # 简化处理：对于离散动作空间
    ...
    def _run_episode(self, env: EnvWrapper, agent: DQNAgentBase) -> Dict:
        """运行一个 episode"""
        state, _ = env.reset()
        score = 0
        done = False
    ...
    def save_checkpoint(self, path: str, agent: DQNAgentBase):
        """保存检查点"""
        agent.save(path)

    def load_checkpoint(self, path: str, agent: DQNAgentBase):
        """加载检查点"""
        agent.load(path)


# 保持向后兼容的别名
DQNTrainer = DQNTrainerBase
